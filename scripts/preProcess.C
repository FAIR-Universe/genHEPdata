...
void AnalyseEvents(ExRootTreeReader *treeReader, string outputFile_part,const int label){
    ...
    // string output_filetype =  ".root";
    // string index = to_string(begin_index);
    ...
    for(entry = 0; entry < allEntries; ++entry){
        ...
        if((flag_el > 1)||(flag_mu > 1)){continue;}

        if(flag_mu == flag_el){continue;}
        h_cutflow ->Fill(2);

        if(pt_lep < 20){continue;}
        h_cutflow ->Fill(3);
        ...
        if(flag_had!=1){continue;}
         h_cutflow ->Fill(4);
        if((charge_had + charge_lep)!=0){continue;}
        h_cutflow ->Fill(5);
        if(pt_had < 20){continue;}
        h_cutflow ->Fill(6);
